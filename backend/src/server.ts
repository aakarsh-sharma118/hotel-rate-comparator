  ...
  helmet({
    contentSecurityPolicy: {
      directives: {
        ...
        connectSrc: ["'self'", 'http://localhost:3001', 'http://localhost:3000', 'http://localhost:5173', 'https://*.onrender.com', 'ws:'],
        objectSrc: ["'none'"],
      },
    },
...
app.get('/', (req: Request, res: Response): void => {
  ...
  if (req.accepts('html') && !req.accepts('json')) {
    const frontendBtn = frontendUrl
      ? `<a href="${frontendUrl}" class="btn btn-primary" target="_blank" rel="noopener noreferrer">Open Web App (Frontend) &rarr;</a>`
      : '';
...
const searchStatusHandler = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    console.warn(`[Server] Workflow status query failed for ${workflowId}:`, error?.message || error);
    res.status(404).json({
